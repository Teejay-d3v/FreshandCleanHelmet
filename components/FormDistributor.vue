<template>
  ...
  >
    <!-- Modal Content -->
    <div class="relative w-full max-w-7xl p-6 md:p-8 bg-white rounded-lg shadow-lg overflow-y-auto max-h-[90vh]">
      <button
        @click="emitClose"
      ...
      <form class="space-y-6" @submit.prevent="handleSubmit">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          ...
          <!-- Business Name -->
          <div>
            <label for="business_name" class="block text-lg font-medium text-gray-700">
              Business Name
            </label>
            <input
              type="text"
...
async function handleSubmit(event) {
  ...
  try {
    const response = await fetch('https://script.google.com/macros/s/AKfycbzSS9oFTyHVVDonbyrKxLgcogHk2adlloOXEaxxp8RtVwwhWcCjX46QpgoKBgoSmtK8/exec', {
      method: 'POST',
